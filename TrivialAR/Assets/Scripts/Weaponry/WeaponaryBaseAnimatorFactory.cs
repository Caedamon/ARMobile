...
public static class WeaponaryBaseAnimatorFactory
{
    ...
    public static void CreateBaseAnimator()
    {
        ...
        var sIdle = sm.AddState("Idle", new Vector3(300, 100)); sIdle.motion = idle;
        var sWalk = sm.AddState("Walk", new Vector3(550, 100)); sWalk.motion = walk;
        var sAtkL = sm.AddState("Attack_Light", new Vector3(300, 260)); sAtkL.motion = aL; sAtkL.speed = 1f; sAtkL.speedParameterActive = false;
        var sAtkH = sm.AddState("Attack_Heavy", new Vector3(550, 260)); sAtkH.motion = aH;
        var sBlk  = sm.AddState("Block", new Vector3(800, 260)); sBlk.motion = blk; sBlk.writeDefaultValues = true;
        var sHit  = sm.AddState("Hit", new Vector3(300, 420)); sHit.motion = hit;
        var sDie  = sm.AddState("Die", new Vector3(550, 420)); sDie.motion = die; sDie.speed = 1f; sDie.iKOnFeet = false;
    ...
    public static void AssignToSelected()
    {
        ...
        if (!obj) { Debug.LogWarning("Select a character GameObject."); return; }
        ...
        if (!ctrl)
        {
            ...
            if (guids.Length > 0) ctrl = AssetDatabase.LoadAssetAtPath<AnimatorController>(AssetDatabase.GUIDToAssetPath(guids[0]));
        }
        if (!ctrl) { Debug.LogWarning("Create the base controller first (Assets/Create/Weaponary/Create Base Melee Animator)."); return; }
